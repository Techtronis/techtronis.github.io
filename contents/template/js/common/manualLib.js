...
TemplateSettings.smallSubtitleDivId = "small_subtitle";
...
TemplateSettings.styleSmallSubtitleDiv = "small_subtitle";
...
TemplateSettings.apiIframePadW = 0;
TemplateSettings.apiIframePadH = 6;
...
function TocDataObject(type, category, level, url, id, text, baselevel, figure, table, code, hierarchy, visible)
{
  this.dataType = type;
...
function TocObject()
{
  ...
  this.getPageNaviObjectId = function(url, hierarchy)
  {
    ...
    if (pos >= 0)
    {
      if (url.match(/external\.html\?file=/))
      {
        url = url.substr(pos + "?file=".length);
        var lpos = url.lastIndexOf("&");
        if (lpos != -1)
        {
          url = url.substr(0, lpos)
        }
      }
      else
      {
  ...
  this.setSmallSubtitle = function(title)
  {
    this.smallSubtitle = ((title == null) ? "" : title)
  };
  this.getSmallSubtitle = function()
  {
  ...
  this._addTocDataObjectCore = function(type, category, level, name, url, id, noPrint, hierarchy, visible)
  {
    ...
    if (this._checkSameData(hie))
      return;
    var data = new TocDataObject(type, category, level, url, id, name, this.baselevel, this.figureNo, this.tableNo, this.codeNo, hie, visible);
  ...
  this.addSeparator = function(hierarchy, level)
  {
    if (typeof level === 'undefined')
    {
      level = hierarchy.split("_").length - 1
    }
    this._addTocDataObjectCore("SEPARATOR", "", level, "", "", "", true, hierarchy)
  };
  this.addBlankLine = function(hierarchy, level)
  {
    if (typeof level === 'undefined')
    {
      level = hierarchy.split("_").length - 1
    }
    this._addTocDataObjectCore("BLANK", "", level, "", "", "", true, hierarchy)
  };
  this.addExternal = function(category, level, name, url, hierarchy, visible)
  {
    this._addTocDataObjectCore("EXTERNAL", category, level, name, url, "", true, hierarchy, visible)
  };
  ...
  this._dataTypeSettings = {
    MAN_HEADER: [true, true, false, false], MAN: [true, true, false, false], HEADER: [true, true, false, false], ITEM: [true, true, false, false], GUIDELINE_SECTION: [true, true, false, false], GUIDELINE_HEADER: [true, true, false, false], GUIDELINE: [true, true, false, false], SEPARATOR: [false, false, false, false], EXTERNAL: [false, true, true, false], EXTERNAL_HEADER: [false, false, true, false], MAN_EXTERNAL: [false, false, true, false], MAN_EXTERNAL_HEADER: [false, false, true, false], RAWURL: [false, false, true, true], RAWURL_HEADER: [false, false, true, true], MAN_RAWURL: [false, false, true, true], MAN_RAWURL_HEADER: [false, false, true, true], NOURL_HEADER: [false, false, false, false], MAN_NOURL_HEADER: [false, false, false, false]
  };
  ...
  this.processTreeData = function()
  {
    ...
    for (var i = 0; i < this.getDataCount(); i++)
    {
      ...
      {
        arrayLevel[currentLevel] = arrayLevel[currentLevel] - 1;
        if (arrayLevel[currentLevel] < 0)
          arrayLevel[currentLevel] = 0
      }
      for (var n = 0; n <= this._maxLevel; n++)
      {
        if (arrayLevel[n] > 0)
  ...
  this.createContents = function()
  {
    ...
    var smallsubtitle = this.getSmallSubtitle();
    {
      var elem = document.getElementById(TemplateSettings.smallSubtitleDivId);
      if (elem != null)
      {
        var obj = document.createElement("SPAN");
        obj.innerHTML = smallsubtitle;
        elem.appendChild(obj)
      }
    }
...
Reassemble.reassembleImpl = function()
{
  ...
  if (!forceNoIndex && ((typeof jQuery) === "function") && ($(TemplateSettings.selectorTocFile).attr("src") != null) && (Reassemble.objectTocData != null))
  {
    ...
    $("#" + Reassemble.globalindexRootPaneId).html('<div id="' + TemplateSettings.titleLogoDivId + '" class="' + TemplateSettings.styleTitleLogoDiv + '" align="center"></div>' + '<div id="' + TemplateSettings.titleTocDivId + '" class="' + TemplateSettings.styleTitleTocDiv + '" align="center"></div>' + '<div id="' + TemplateSettings.smallSubtitleDivId + '" class="' + TemplateSettings.styleSmallSubtitleDiv + '" align="center"></div>' + '<div id="' + TemplateSettings.searchBoxMiniDivId + '" align="center"></div>' + '<div id="' + TemplateSettings.printControllerId + '" align="left"></div>' + '<div id="' + TemplateSettings.treeControllerId + '"></div>' + '<div id="' + TemplateSettings.divTreePaneId + '" class="' + TemplateSettings.styleTreePane + '">' + '</div>');
...
Reassemble.getBreadCrumbText = function(manager, current)
{
  ...
  while (pobj != null)
  {
    var delim = (pobj.currentUrl === current.currentUrl) ? '' : Reassemble.escapeHtml(' > ');
    if (pobj.linkEnabled && pobj.currentUrl != "")
    {
      var externalPrefix = (pobj.dataType.match(/external/i)) ? 'external.html?file=' : '';
      breadcrumbtext = '<a href="' + (manager.isRawUrlLink(pobj.dataType) ? '' : externalPrefix + manager.getRootURL()) + pobj.currentUrl + '"' + (manager.isExternalLink(pobj.dataType) ? ' target="_blank"' : '') + '>' + Reassemble.escapeHtml(pobj.title) + '</a>' + delim + breadcrumbtext
    }
    else
    {
      breadcrumbtext = Reassemble.escapeHtml(pobj.title) + delim + breadcrumbtext
    }
...
Reassemble.viewImage = function(obj)
{
  ...
  if (tempObj != null)
  {
    ...
    {
      temp.unbind("load");
      var orgHeight = imageHeight * 1;
      var orgWidth = imageWidth * 1;
      ...
      $(Reassemble.viewerElement).css({
        position: "fixed", float: "left", zIndex: "9999", margin: "0px", "overflow-x": "auto", "overflow-y": "auto", cursor: "pointer", backgroundColor: "#ffffff", padding: "8px", border: "2px solid #000000", borderRadius: "8px", boxShadow: "rgba(0,0,0,0.5) 4px 4px", "text-align": "center"
      });
...
Search.indexData = [];
Search.search = function(query, msgId, dstId, flags, skip)
{
  var enableAddon = false;
  var loader = new DynLoader(Search.searchImpl, false);
  loader.option = {
    query: query, msgId: msgId, dstId: dstId, flags: flags, skip: skip
  };
  if ((typeof AddonList !== "undefined") && $.isArray(AddonList))
  {
    for (var n = 0, len = AddonList.length; n < len; n++)
  ...
  {
    Search.goSearch(query, msgId, dstId, flags, skip)
  }
};
Search.searchImpl = function(param)
{
  Search.goSearch(param.option.query, param.option.msgId, param.option.dstId, param.option.flags, param.option.skip)
};
Search.getPageType = function(data, query)
{
  if (data[3] == 1)
    return "api";
  if (data[3] == 2)
  ...
  if (data[3] == 3)
    return "clog";
  var url = Search.addQueryToURL(data[0], query, "highlighttext");
  if ((url.length > 1) && (url[0] == "/"))
    url = url.slice(1);
  var pageObj = Reassemble.getCurrentPageNaviObjectByUrl(Reassemble.objectTocData, url);
  if (pageObj != null)
  {
    var breadcrumb = Reassemble.getBreadCrumbText(Reassemble.objectTocData, pageObj);
    if (breadcrumb.match(/(既知の不具合|Known Bugs)/))
      return "known"
  }
  return "others"
};
Search.goSearch = function(query, msgId, dstId, flags, skip)
{
  ...
  for (var n = 0; n < Search.indexData.length; n++)
  {
    ...
    if (data.length < 4)
      continue;
    var type = Search.getPageType(data, query);
    switch (type)
    {
      case"api":
        if (!flags.api)
